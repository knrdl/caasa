<script lang="ts">
    ...
    onMount(() => {
        api.register<{ username: string }>('login', ({username}) => {
...
<div class="container pt-5">
    ...
    <form class="container login-form" on:submit|preventDefault={() => login()} transition:slide|local>
        <div class="form-floating mb-2">
            <input type="text" class="form-control" id="loginName" placeholder=" " bind:value={username}
                   on:input={() => {username = username.toLowerCase().trim()}} required autocomplete="username">
            <label for="loginName">Username</label>
        </div>
        <div class="form-floating mb-2">
            <input type="password" class="form-control" id="loginPw" placeholder=" " bind:value={password} required
