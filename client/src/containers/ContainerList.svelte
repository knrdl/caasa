...
<aside class="d-flex flex-column pb-1">
    {#if containers}
        ...
        {:else}
            <input type="search" class="form-control" placeholder="Search" value={search}
                   on:input={e=>search=e.target.value.toLowerCase()}>
            {#each filteredContainers as container}
                ...
                <div class="d-flex border-top border-bottom" style="cursor: pointer"
                     ...
                     class:bg-dark={isSelected} class:text-light={isSelected}>
