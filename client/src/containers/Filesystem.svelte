<script lang="ts">
    ...
    beforeUpdate(() => {
        if (container_id !== oldContainerId) {
            ...
            info = null
            if (container_id)
                api.send('get_filesystem_info', {container_id})
        }
    })

    onMount(() => {
        api.register<FilesystemInfo>('get_filesystem_info', response => {
            loadPath(response.workdir)
            info = response
        }, (err) => messageBus.add({text: err, type: 'error'}))
        api.register<{ path: string, entries: DirectoryListing[] }>('get_directory_list', response => {
        ...
        api.register<{ payload: { path: string }, blob: Blob }>('download_file', response => {
            downloadBlob(response.blob, response.payload.path.split('/').pop())
        }, (err) => messageBus.add({text: err, type: 'error'}))
        api.register<void>('create_folder', () => {
            loadPath(currentDir)
        }, (err) => messageBus.add({text: err, type: 'error'}))
        api.register<void>('upload_file', () => {
            loadPath(currentDir)
        }, (err) => messageBus.add({text: err, type: 'error'}))
    })
    ...
    function requestFileDownload(filename: string) {
        if (allow_download)
            api.send('download_file', {container_id, path: `${currentDir}/${filename}`})
    }
    ...
    {:else}
            ...
            <div class="d-flex">
                ...
                <div class="btn-group mx-2" role="group">
                    {#each parseCurrentDir() as itm}
                        <button type="button" class="btn btn-secondary border-dark"
                                on:click={()=> loadPath(itm.totalPath)}>{itm.segment || '/'}</button>
                    {/each}
                </div>
            </div>
        ...
        <table class="table table-hover">
            ...
            {#each (currentEntries || []) as e}
                <tr class:clickable={e.type === 'd' || allow_download}
                    on:click={()=>e.type === 'd' ? gotoSubDir(e.name) : requestFileDownload(e.name)}>
                    <td>
                        {#if e.type === 'd'}
