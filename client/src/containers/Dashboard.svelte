<script lang="ts">
    ...
    onMount(() => {
        ...
        api.register<ContainerInfoLong>('get_container_info', info => {
        ...
        api.register<void>('set_container_state', () => {
            runningAction = null
        }, err => messageBus.add({text: err, type: 'error'}))
    ...
    {:else}
        <div class="d-flex mb-3">
            ...
            <div class="d-flex flex-column flex-grow-1">
                        ...
                        <div class="ms-4">
                            {#if container.status === 'running'}
                                {#if !runningAction || runningAction === 'restart'}
                                    <button type="button" class="btn btn-outline-warning" disabled={!!runningAction}
                                ...
                                {#if !runningAction || runningAction === 'stop'}
                                    <button type="button" class="btn btn-outline-danger" disabled={!!runningAction}
                                ...
                                {/if}
                            {:else}
                                {#if !runningAction || runningAction === 'start'}
                                    <button type="button" class="btn btn-outline-success" disabled={!!runningAction}
                ...
                <div>
                    <Fa icon={faTerminal}/>
                    <code>{container.command}</code></div>
            </div>
        </div>
                ...
                <div>
                    <h6>
                        {#if !image_name.includes('/')}
                            <a href="https://hub.docker.com/_/{image_name}" target="_blank"
                               rel="noopener noreferrer" class="text-decoration-none">{image_name}</a>
                        {:else if image_name.match(/^\w+\/\w+$/)}
                            <a href="https://hub.docker.com/r/{image_name}" target="_blank"
                               rel="noopener noreferrer" class="text-decoration-none">{image_name}</a>
                        {:else}
                            {image_name}
                        {/if}
